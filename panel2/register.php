...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$captcha->validateCaptcha($captchaInput)) {
        ...
        if ($stmt->num_rows > 0) {
			...
			$encodedPass = password_hash($password, PASSWORD_BCRYPT);
			// Substituir $2y$ por $2a$ no hash gerado
			$encodedPass = str_replace('$2y$', '$2a$', $encodedPass);

            $stmt = $conn->prepare("INSERT INTO accounts (login, password, access_level) VALUES (?, ?, 0)");
