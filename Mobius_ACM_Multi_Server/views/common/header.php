...
?>
		<ul class="navbar-nav bg-gradient-primary sidebar sidebar-dark accordion" id="accordionSidebar">
			...
			<div class="status nav-item">
				...
				<?php } 
				if($settings->check('show_game_status')){
					foreach($gameservers as $g){
						echo '
				<div class="nav-link pb-0">
					<span>'.$g['name'].': '.getPortStatus($g['host'], $g['port']).'</span>
				</div>';
					}
				} 
				?>
                ...
                <nav class="navbar navbar-expand navbar-light bg-white topbar mb-4 static-top shadow">
                    ...
                    <ul class="navbar-nav ml-auto">
						...
						<div class="dropdown d-flex justify-content-end">
							<button class="btn btn-primary dropdown-toggle" type="button" data-toggle="dropdown" aria-expanded="false" style="border-radius:0"><?=$gameservers[$selected_gameserver]['name'];?></button>
							<div class="dropdown-menu">
							<?php foreach($gameservers as $key=>$val){
								echo '
								<a class="dropdown-item change-gameserver" href="#" data-id="'.$key.'">'.$val['name'].'</a>';
							} ?>
							</div>
						</div>
